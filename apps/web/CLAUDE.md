...
| Formatting | `formatting.ts` | `formatBucket(bucket, granularity, compact?)`, `formatSeconds(s)`, `formatRelativeTime(iso)`, `eventBadgeVariant(eventName)`, `formatGranularity(count, granularity)`, `formatCompactNumber(n)` | Shared formatting for chart axes, durations, relative timestamps, event badge variants, and locale-aware granularity pluralization (day/week/month). Use `formatCompactNumber` as `tickFormatter` on YAxis in compact/dashboard chart mode — renders `<1000` as-is, `>=1000` as `"1.2K"`, `>=1M` as `"4.5M"`. Import instead of defining local formatters. `compact` mode produces shorter labels for dashboard widgets |
