...
import { ProjectTokenTab } from './project-token-tab';
import { BillingTab } from './billing-tab';
import { IngestionWarningsTab } from './ingestion-warnings-tab';

type TabId = 'general' | 'members' | 'token' | 'billing' | 'warnings';

export default function SettingsPage() {
  const projectId = useProjectId();
  const VALID_TABS = ['general', 'members', 'token', 'billing', 'warnings'] as const;
  ...
  const tabs = useMemo(() => [
    ...
    { id: 'token' as const, label: t('projectToken') },
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'token' && <ProjectTokenTab projectId={projectId} />}
