...
export function CompactLegend({ allSeriesKeys, formulaKeys, hiddenKeys }: CompactLegendProps) {
  return (
    <div className="flex flex-wrap gap-x-3 gap-y-0.5 pt-1.5 px-1">
      {allSeriesKeys.map((key, idx) => {
        const isHidden = hiddenKeys?.has(key);
        return (
          <div key={key} className={`flex items-center gap-1 min-w-0 ${isHidden ? 'opacity-40' : ''}`}>
            <span
