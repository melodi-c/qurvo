...
import translations from './NotPerformedEventSequenceRow.translations';
import type { NotPerformedEventSequenceCondition } from '../types';

interface NotPerformedEventSequenceRowProps {
  condition: NotPerformedEventSequenceCondition;
  onChange: (condition: NotPerformedEventSequenceCondition) => void;
  onRemove: () => void;
}

export function NotPerformedEventSequenceRow({ condition, onChange, onRemove }: NotPerformedEventSequenceRowProps) {
  ...
  return (
    <div className="rounded-lg border border-border/70 bg-muted/20 p-3 space-y-2.5">
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-semibold uppercase tracking-wider text-red-400">{t('notPerformedEventSequence')}</span>
