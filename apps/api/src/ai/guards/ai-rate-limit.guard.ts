...
import { AI_RATE_LIMIT_PER_MINUTE, AI_RATE_LIMIT_PER_HOUR } from '../../constants';
...
export class AiRateLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const minuteBucket = Math.floor(now / MINUTE_WINDOW_SECONDS);
    const hourBucket = Math.floor(now / HOUR_WINDOW_SECONDS);
    ...
    const [minuteCount, hourCount] = await this.redis
      .pipeline()
      .incr(minuteKey)
      .expire(minuteKey, MINUTE_WINDOW_SECONDS * 2)
      .incr(hourKey)
      .expire(hourKey, HOUR_WINDOW_SECONDS * 2)
      .exec()
      .then((results) => {
