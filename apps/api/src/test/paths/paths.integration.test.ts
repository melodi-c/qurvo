...
describe('queryPaths — timezone support', () => {
  it('shifts day boundary with Europe/Moscow timezone (UTC+3)', async () => {
    ...
    // Insert events at 23:00 UTC (day D) — this is 02:00 MSK on day D+1.
    const now = new Date();
    ...
    // Without timezone: events at 23:00-23:10 UTC fall in day D.
    const withoutTz = await queryPaths(ctx.ch, {
      project_id: projectId,
    ...
    expect(withoutTz.transitions[0].source).toBe('pageview');
    expect(withoutTz.transitions[0].target).toBe('signup');

    // With timezone=Europe/Moscow: events are at 02:00 MSK on D+1 — NOT in day D.
    const withMskDayD = await queryPaths(ctx.ch, {
      project_id: projectId,
      date_from: utcDateStr,
      date_to: utcDateStr,
      step_limit: 5,
    ...
    expect(withMskDayD.transitions).toHaveLength(0);

    // With timezone=Europe/Moscow and querying D+1: events SHOULD appear.
    const withMskDayD1 = await queryPaths(ctx.ch, {
      project_id: projectId,
    ...
    expect(withMskDayD1.transitions).toHaveLength(1);
    expect(withMskDayD1.transitions[0].source).toBe('pageview');
    expect(withMskDayD1.transitions[0].target).toBe('signup');
  });
