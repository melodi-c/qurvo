...
Входные данные: `WORKTREE_PATH`, `ISSUE_NUMBER`, `ISSUE_TITLE`, `ISSUE_DATA_FILE` (путь к JSON-файлу с body, comments, labels), `AFFECTED_APPS`, `BASE_BRANCH` (по умолчанию `main`). Ревьюер сам выполняет `git diff` в worktree для получения списка изменений.
...
### 2.9 Бесполезные комментарии

Ищи в diff добавленные комментарии (`//`, `/* */`, `/** */`), которые не несут ценности:

- **MAJOR** — комментарий содержит неверную или устаревшую информацию (ссылается на код/паттерн, которого нет: "repeats 6+ times", "removed in v2", "see old implementation")
- **MAJOR** — комментарий-призрак после удалённого кода (описывает логику, которой больше нет в файле)
- **MAJOR** — комментарий просто пересказывает код (`// increment counter` над `counter++`, `// returns the result` над `return result`)
- **WARNING** — JSDoc, который дублирует сигнатуру без добавления смысла (`@param name — the name`, `@returns the result`)

НЕ считать нарушением:
- Комментарии, объясняющие **почему** (бизнес-причина, workaround, неочевидное поведение)
- `// TODO`, `// FIXME`, `// HACK` с объяснением
- Лицензионные заголовки
- `eslint-disable` с причиной

При обнаружении — `action: "DELETE"`, в `suggested_code` пустая строка.

### 2.10 Scope creep

Изменены файлы/модули которые **не связаны** с задачей:

- **MAJOR** — изменения в файлах/директориях другого приложения (например, issue для `apps/api` но изменён `apps/web`), добавление новых фич не описанных в issue, рефакторинг не связанный с задачей
- **WARNING** — косметические изменения в связанных файлах (форматирование, переименование), незначительные улучшения в затронутом модуле

Scope creep — самая частая причина rejected PR (по данным индустрии ~25%). Будь строг: если изменение не требуется для решения задачи — это scope creep.
