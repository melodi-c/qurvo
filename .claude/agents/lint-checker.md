...
### 2.7 Unsafe type casts

В добавленных строках (не тестовых файлах) ищи:
- `as unknown as` — **FAIL** (двойной каст, обход системы типов)
- `as any` — **FAIL**
- `as never` — **FAIL**

Исключения: тестовые файлы (`*.test.*`, `*.spec.*`, `*.integration.*`), файлы деклараций (`*.d.ts`).
