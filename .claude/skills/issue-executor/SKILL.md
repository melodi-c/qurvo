...
**Фильтр `skip`**: после получения списка исключи все issues с лейблом `skip` — они намеренно пропущены пользователем. Если такие есть — упомяни их в отчёте как "пропущено (skip)".

**Автовалидация**: если среди оставшихся issues есть те, у которых нет лейбла `ready` — автоматически запусти агента `issue-validator` в foreground перед стартом:

```
subagent_type: "issue-validator"
run_in_background: false
prompt: |
  Проверь issues: #N #M ... (перечисли номера без лейбла ready)
```

Дождись завершения валидатора. Если после валидации у каких-то issues появился лейбл `needs-clarification` — сообщи пользователю и спроси:
> ⚠️ Issues #N, #M получили `needs-clarification` — acceptance criteria недостаточны.
> Продолжить без них? (да / нет)

При `нет` — остановись. При `да` — исключи эти issues из списка и продолжи.
...
1. **Навесь лейбл `in-progress`** на все issues группы перед запуском и добавь комментарий:
   ```bash
   gh issue edit <NUMBER> --add-label "in-progress"
   gh issue comment <NUMBER> --body "🔄 **Взято в работу агентом.** Реализация запущена в изолированном worktree."
   ```
...
- `STATUS: SUCCESS` — подагент реализовал задачу, issue закрыт. Оркестратор делает мерж:
  ...
  6. **ВАЖНО**: после `worktree remove` перейди в `$REPO_ROOT` перед любыми `gh` командами — иначе `gh` внутри вызывает `git` с CWD удалённой директории и падает с `fatal: Unable to read current working directory`:
     ...
     cd "$REPO_ROOT"
     ```
  7. **Добавь итоговый комментарий на issue** (issue уже закрыт solver'ом — комментарий добавится к закрытому):
     ```bash
     ...
     **Review**: REVIEW_LINE
     ...
     ```
     Подставь значения из строк `FILES:`, `TESTS:`, `TSC:`, `REVIEW:` результата подагента. Если строк нет — подставь "—".
  8. Проверь что целевая ветка продвинулась (`BASE_BEFORE != BASE_AFTER`) — если нет, считай FAILED.
...
- **#45**: уточнить acceptance criteria → обнови описание issue, затем `/issue-executor 45` (автовалидация запустится автоматически)
```
