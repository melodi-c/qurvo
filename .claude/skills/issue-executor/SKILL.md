...
1. **Запусти `start-group.sh`**:
   ...
   ```
2. Запусти всех подагентов группы **одновременно** (`run_in_background: true`, `subagent_type: "issue-solver"`, **`isolation: "worktree"`**). Ответ Task tool сразу содержит `WORKTREE_PATH` и `task_id` — сохрани оба в state (см. ниже). **НЕ вызывай TaskOutput** — система сама уведомит о завершении каждого подагента.

**ВАЖНО — сохрани task_id**: Сразу после запуска каждого solver'а сохрани его `task_id` в state:
   ```bash
   bash "$SM" task-id <NUMBER> <TASK_ID>
   ```
   Это нужно для recovery после compact — executor сможет проверить, жив ли agent, вместо ложного рестарта.
...
**Лимит параллелизма**: Запускай НЕ БОЛЕЕ 10 solver-агентов одновременно. Если в группе >10 issues — разбей на под-батчи по 10 и выполняй под-батчи последовательно.
...
5. Лимит параллелизма: если SOLVING issues >= 10, оставь новый в PENDING — запустится после освобождения слота.
