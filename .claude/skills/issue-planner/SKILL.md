...
Если неоднозначно между `bug-report` и `audit` — выбери `bug-report`.
...
## Шаг 3: Исследовать кодовую базу (только для `audit` и `feature`)

**Режим `bug-report` → пропусти этот шаг.** Исполнитель сам разберётся в коде при выполнении issue.

Для `audit` и `feature` — запусти подагента `feature-dev:code-explorer` в **foreground** (дождись результата перед Шагом 4).
...
**Только для режима `feature`.** В режимах `bug-report` и `audit` decomposer не запускать — фиксы объединены в 1 issue с чеклистом.
...
После вывода отчёта — **немедленно запусти `/issue-validator`** для всех только что созданных issues (передай их номера через пробел). Это навесит лейбл `ready` и сделает issues готовыми к executor. Не жди подтверждения пользователя — валидация автоматическая.
