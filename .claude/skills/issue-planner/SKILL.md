...
Ты — ТОЛЬКО планировщик и создатель issues. ЗАПРЕЩЕНО:
- Использовать **Edit, Write, NotebookEdit** для любых файлов
- Запускать `git commit`, `git push`, `git checkout` и другие мутирующие git-команды
- Запускать тесты, build, миграции
- Вносить исправления в код — даже если проблема очевидна. Твоя задача — **описать** проблему в issue, а не решать её

Допустимые действия:
- Read, Grep, Glob — для исследования кодовой базы
- Bash — только для `gh issue`, `gh label`, `gh api`, `gh repo view`, `gh pr list`
- Agent (Explore, feature-dev:code-architect, issue-decomposer) — для анализа кода
...
**`refactor`** — переработка существующего кода без изменения поведения:
- слова: "рефакторинг", "переписать", "переехать на", "заменить X на Y", "migrate to", "вынести в", "убрать дублирование", "refactor", "rewrite", "extract", "consolidate", "simplify"
- описание желаемой архитектуры/паттерна без нового функционала
- отличие от `audit`: пользователь знает **что** хочет изменить, не просит **искать** проблемы
...
Если промпт содержит явное указание на приложение/пакет (например, "в API", "на фронте", "в processor") — используй его как scope.

Если scope не указан и режим `audit` или `refactor` — спроси пользователя:
> В каких приложениях искать? (все / api / web / ingest / processor / cohort-worker / billing-worker / конкретный пакет)

Для `bug-report` и `feature` scope обычно понятен из контекста — спрашивай только если действительно неоднозначно.

Передай scope в промпт подагента (Шаг 3) для фокусировки исследования.
...
Также проверь недавно закрытые issues (могут быть закрыты как won't fix, reverted, или дубликат):
```bash
gh issue list --state closed --json number,title,closedAt --limit 20
```
Если среди закрытых есть похожий по теме — отметь в preview (Шаг 5): `⚠ Похожий issue #N был закрыт <дата>`. Пользователь решит, создавать ли новый.

Проверь открытые PR на предмет пересечений:
```bash
gh pr list --state open --json number,title,headRefName --limit 20
```
Если есть PR, затрагивающий ту же область:
- В preview (Шаг 5) добавь: `⚠ Related PR: #N (название)`
- В теле issue добавь секцию `## Related PRs` перед `## Agent Context`:
  ```
  ## Related PRs
  - #N — [название PR, что он меняет в этой области]
  ```
  Это помогает solver'у учесть in-flight изменения и избежать конфликтов.
...
### Режим `audit`, `refactor` и `feature` — полное исследование

Запусти подагента в **foreground** (дождись результата перед Шагом 4).

Выбор агента:
- `audit` и `refactor` → subagent_type: `"Explore"`
- `feature` → subagent_type: `"feature-dev:code-architect"` (архитектурный анализ с blueprint реализации)
...
### Режим `refactor`:
```
Проведи анализ кодовой базы для планирования рефакторинга:
...
Сфокусируйся на:
1. Текущее состояние: как устроен код сейчас (файлы, паттерны, зависимости)
2. Целевое состояние: как должно быть после рефакторинга
3. Стратегия миграции: в каком порядке менять, чтобы не сломать промежуточные состояния
4. Риски: что может сломаться, какие тесты нужно обновить
5. Зависимости: что нужно сделать раньше (DB schema → backend → frontend)

Верни конкретные файлы и план шагов, не абстрактные описания.
```

### Режим `feature` (агент: `feature-dev:code-architect`):
```
Проведи архитектурный анализ для реализации следующей фичи:
...
Сфокусируйся на:
1. Изучи существующие аналогичные реализации в кодовой базе — используй их как шаблон
2. Определи какие файлы/модули нужно создать и какие расширить
3. Для каждого нового файла — покажи примерную структуру (интерфейсы, методы)
4. Выяви зависимости между частями (что нужно сделать раньше)
5. Предложи разбивку на атомарные задачи (backend / frontend / DB / тесты)
6. Для каждой задачи — конкретные файлы и что именно изменить/добавить
7. Если задачи образуют единую фичу — предложи эпик с acceptance criteria

Верни blueprint реализации с конкретными файлами и аналогами из кодовой базы.
```

Если `feature-dev:code-architect` недоступен — используй `"Explore"` как fallback с тем же промптом.
...
- **Не дублируй**: если похожая задача уже есть в открытых или недавно закрытых issues — пропусти
- **Лимит**: максимум **10 issues** за один вызов. Если находок больше — отбери топ-10 по приоритету (P1 > P2 > P3) и сообщи в preview: `Найдено N проблем, показаны топ-10. Ответь "ещё" для следующей порции.` При ответе `ещё` — покажи следующие 10
...
**Режим `refactor` — тело issue:**
```
## Текущее состояние

[Как устроено сейчас. Конкретные файлы, паттерны, зависимости.]

## Целевое состояние

[Как должно быть после рефакторинга. Целевой паттерн/архитектура.]

## Стратегия миграции

- [ ] `файл:строка` — [что именно изменить и в каком порядке]
- [ ] ...

## Acceptance Criteria

- [ ] [Поведение не изменилось — существующие тесты проходят]
- [ ] [Новый паттерн применён ко всем указанным местам]
...
P2 / P3
```
Labels: `refactor` + scope + `size:xs/s/m/l` + `has-migrations` (если затрагивает DB/CH)
...
**Для режимов `feature`, `audit` и `refactor`** (не для `bug-report` — фиксы багов объединены в 1 issue с чеклистом).

Для каждого сформированного issue (не эпика) в режиме `feature`, `audit` или `refactor` оцени: затрагивает ли он 3+ несвязанных модуля или требует изменений в 2+ приложениях одновременно? Если да — запусти `issue-decomposer` в **foreground**:
...
- Если вернул список `sub_issues` → замени исходный issue на эти sub-issues в черновиках:
  - **Добавь `## Agent Context`** к каждому sub-issue (decomposer его не генерирует). Используй данные из Explore/code-architect исследования (Шаг 3): starting_locus, affected_files, verification, boundaries. Каждый sub-issue должен иметь собственный Agent Context, релевантный его scope
...
## Шаг 4.6: Валидация путей файлов

После формирования всех черновиков (включая sub-issues из decomposer) — проверь, что пути в `Agent Context` реальны.

Для каждого уникального `starting_locus` и каждого файла из `affected_files` — выполни `Glob` или `Read` чтобы убедиться, что файл существует. Если файл не найден:
- Попробуй найти похожий файл через `Glob` (например, `**/events.service.ts`)
- Если нашёл — замени путь на правильный
- Если не нашёл — удали строку из `affected_files` и добавь комментарий `(path not verified)` к `starting_locus`

Номера строк (`:42-80`) не нужно проверять — достаточно что файл существует.

**Оптимизация**: собери все уникальные пути, проверь пакетно через один `Glob` с паттерном, не проверяй каждый файл отдельно.

---

## Шаг 5: Показать preview и получить подтверждение

Обычные issues отображаются с Acceptance Criteria:
```
── 1 ──────────────────────────────────────────
Заголовок: fix(web): ...
Labels:    web, ux/ui, size:s
Приоритет: P2
...
```
────────────────────────────────────────────────
Создать N issues в GitHub? (да / нет / да 1,3,5 / редактировать / подробнее N)
```

**Дождись ответа:**
- `нет` → остановись
- `редактировать` → обсуди правки, покажи обновлённый список
- `подробнее N` → покажи полное тело issue #N (включая Agent Context) в markdown-блоке. После просмотра снова спроси подтверждение
- `да` → создать все issues (Шаг 6)
- `да 1,3,5` → создать только указанные issues (по номерам из preview). **Проверка зависимостей**: если выбранный issue содержит `Depends on: #DRAFT-N` и issue N **не** в списке выбранных — предупреди пользователя:
  ```
  ⚠ Issue #3 зависит от #2, который не выбран.
  Варианты: (добавить #2 / убрать зависимость / отмена)
  ```
  Дождись ответа. Не удаляй зависимости молча
