...
export function buildSequenceCore(
  ...
  const windowMsValue = cond.time_window_days * 86_400_000; // ms
  ...
  cond.steps.forEach((step, i) => {
    ...
    const filterClause = buildEventFilterClauses(step.event_filters, `coh_${condIdx}_s${i}`);
  ...
  const withinWindow = lte(sub(evt1, acc2), namedParam(windowMsPk, 'UInt64', windowMsValue));
